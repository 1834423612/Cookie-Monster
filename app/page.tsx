...
export default function HomePage() {
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-gradient-to-br from-slate-50 via-white to-blue-50/30 text-slate-800">
        ...
        <section className="grid min-h-0 flex-1 gap-4 md:grid-cols-[2fr_1fr]">
          {!isJarOpened ? (
            <button
              ...
              className="rounded-2xl border border-slate-200 bg-white p-8 text-center transition hover:-translate-y-0.5 hover:shadow-xl shadow-sm"
            >
              <div className="mx-auto mb-3 h-44 w-44 rounded-2xl border border-slate-200 bg-gradient-to-br from-blue-50 to-cyan-50 text-8xl leading-44 shadow-inner flex items-center justify-center">
                <Icon icon="mdi:cookie" className="w-24 h-24 text-blue-500" />
              </div>
            ...
            <section className="flex min-h-0 flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="mb-3 grid gap-2 md:grid-cols-[1fr_auto_auto]">
                <input
                  ...
                  className="h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-700 focus:border-blue-300 focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                />
                <select
                  value={preset}
              ...
              <div className="min-h-0 flex-1 space-y-3 overflow-auto pr-1">
                {filteredGroups.map((group) => (
                  <article key={group.domain} className="rounded-xl border border-slate-200 bg-white">
                    <button
                      onClick={() => setExpandedDomain((current) => (current === group.domain ? null : group.domain))}
                      className="flex w-full items-center justify-between px-4 py-3 text-left hover:bg-slate-50 transition-colors rounded-xl"
                    >
                      <div>
                        <p className="font-semibold text-slate-800">{group.domain}</p>
                        <p className="text-xs text-slate-500">
                          {group.total} cookies · High risk {group.highRiskCount} · Protected {group.recommendedKeepCount}
                        </p>
                      </div>
                      <Icon icon={expandedDomain === group.domain ? "mdi:chevron-up" : "mdi:chevron-down"} className="h-5 w-5 text-slate-400" />
                    </button>

                    {expandedDomain === group.domain && (
                      <div className="space-y-2 border-t border-slate-100 px-3 py-3">
                        {group.items.map((cookie) => (
                          <label key={cookie.key} className="flex cursor-pointer items-start gap-3 rounded-lg border border-slate-200 bg-white p-3 hover:bg-slate-50 transition-colors">
                            <input type="checkbox" checked={Boolean(selected[cookie.key])} onChange={() => toggleCookie(cookie.key)} className="mt-1 accent-blue-500" />
                            <div className="min-w-0 flex-1 text-sm">
                              <div className="flex flex-wrap items-center gap-2">
                                <strong className="text-slate-800">{cookie.name}</strong>
                                <span className="rounded-lg bg-slate-100 px-2 py-0.5 text-xs text-slate-600">{cookie.category}</span>
                                <span className={`rounded-lg px-2 py-0.5 text-xs ${cookie.risk === "high" ? "bg-red-100 text-red-700" : cookie.risk === "medium" ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}>
                                  {cookie.risk}
                                </span>
                                {cookie.recommendedKeep && (
                                  <span className="rounded-lg bg-blue-100 px-2 py-0.5 text-xs text-blue-700">Protected (recommended keep)</span>
                                )}
                              </div>
                              <p className="mt-1 text-xs text-slate-500">Path: {cookie.path} · Expiry: {formatExpiry(cookie.expirationDate)}</p>
                              {cookie.reasons.length > 0 && (
                                <p className="mt-1 text-xs text-slate-400">Signals: {cookie.reasons.join("; ")}</p>
                              )}
                            </div>
                          </label>
                        ))}
              ...
              <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
                ...
                <button
                  ...
                  className="mt-3 w-full rounded-lg bg-gradient-to-r from-blue-500 to-cyan-500 px-3 py-2 text-sm font-semibold text-white enabled:hover:from-blue-600 enabled:hover:to-cyan-600 transition-all shadow-md shadow-blue-500/20 disabled:cursor-not-allowed disabled:bg-slate-300 disabled:from-slate-300 disabled:to-slate-300 disabled:shadow-none"
                >
